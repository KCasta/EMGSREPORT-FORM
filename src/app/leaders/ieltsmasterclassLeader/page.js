...
export default function IeltsMasterclassLeaderDashboard() {
  const [reports, setReports] = useState([]);
  const [loading, setLoading] = useState(true);
  const [selectedReport, setSelectedReport] = useState(null);
  const [refreshKey, setRefreshKey] = useState(0);

  /** Fetch reports for IELTS Masterclass department */
  useEffect(() => {
    async function fetchReports() {
      try {
        const res = await fetch(
          "/api/workerReport?department=IELTS Masterclass"
        );
  ...
  /** Step 4 placeholder rating system */
    ...
    <div className="min-h-screen bg-white text-red-900 py-10 px-4 sm:px-8 font-inter">
      ...
      >
        <h1 className="text-3xl font-bold uppercase mb-2">
          IELTS Masterclass Leader Dashboard
        </h1>
      ...
      {!loading && reports.length === 0 && (
        <p className="text-center text-gray-600 italic">
          No reports submitted yet for IELTS Masterclass department.
        </p>
      )}
